...
- [13. 原子操作（0.11 终极简洁）](#13-原子操作011-终极简洁)
- [14. 内存安全（0.11 强制）](#14-内存安全011-强制)
- [15. 并发安全（0.11 强制）](#15-并发安全011-强制)
- [16. 标准库（0.11 最小集）](#16-标准库011-最小集)
...
- [21. 字符串与格式化（0.11）](#21-字符串与格式化011)
- [22. 未实现/将来](#22-未实现将来)
- [23. 一句话总结](#23-一句话总结)
- [24. 优雅 v0.12 泛型增量文档（未来特性）](#24-优雅-v012-泛型增量文档向后-100-兼容零新关键字无-where)
- [25. 优雅 v0.12 显式宏（可选增量）](#25-优雅-v012-显式宏可选增量)
- [术语表](#术语表)
...
- 空指针字面量：`null`，类型为 `byte*`
  ...
  - 0.11 不支持将 `null` 赋值给 `byte*` 类型的变量（后续版本可能支持）
- 字符串字面量：
...
| `extern` 函数   | C 函数声明    | -         | 0.11 不支持函数指针类型，`extern` 仅用于声明外部 C 函数，[见 5.2](#52-外部-c-函数ffi) |
...
- **编译期常量表达式**：
  ...
  - 比较运算：`==`, `!=`, `<`, `>`, `<=`, `>=`（如果操作数都是常量）
  - 逻辑运算：`&&`, `||`, `!`（如果操作数都是常量）
  - 不支持：函数调用、变量引用（非常量）、数组/结构体字面量（0.11）
- **类型推断**：0.11 不支持类型推断，所有变量必须显式类型注解
- **变量遮蔽**：
  - 同一作用域内不能有同名变量
...
- **函数指针**：0.11 不支持函数指针类型，函数名不能作为值传递或存储，仅支持直接函数调用
- **变参函数调用**：参数数量必须与 C 函数声明匹配（编译期检查有限）
- **程序入口点**：必须定义 `fn main() i32`（程序退出码）
  - 0.11 不支持命令行参数（后续版本支持 `main(argc: i32, argv: byte**)`）
- **`return` 语句**：
...
- **错误处理**（0.11）：
  - 0.11 支持**错误联合类型** `!T` 和 **try/catch** 语法，用于函数错误返回
...
- 字符串插值（第 21 章）是优雅语言本身的特性，编译期展开，与 FFI 的格式字符串不同
...
| 限制 | 说明 |
|---|---|
| 无字段接口 | `struct S { w: IWriter }` → ❌ 编译错误（0.11 限制） |
...
由于优雅 0.11没有泛型，迭代器接口需要针对具体元素类型定义。以下以 `i32` 类型为例：
...
- 需要为每种元素类型定义对应的迭代器接口（0.11 限制，泛型功能在 v0.12 中提供）
...
- 数组大小 `N` 必须是**编译期常量表达式**：
  ...
  - 常量算术运算：`2 + 3`, `SIZE * 2`
  - 不支持函数调用（除非是 `const fn`，0.11 暂不支持）
- **语法规则**：
  - `[]` 表示栈分配的未初始化数组
...
- **`break` 和 `continue`**：
  ...
  - 0.11 不支持 `break label` 或 `continue label`（后续版本支持）
...
- **`for` 循环**：Zig风格的迭代循环，通过接口机制支持所有可迭代类型
  - **语法形式**：
    ...
    - `index_range`：索引范围表达式
      ...
      - `start..end`：从 `start` 到 `end-1` 的范围（0.11 暂不支持带结束值的范围）
...
- **类型比较规则**：
  ...
  - 错误类型支持 `==` 比较（0.11）
  - 0.11 不支持结构体的 `==` 和 `!=` 比较（后续版本支持）
  - 0.11 不支持数组的 `==` 和 `!=` 比较（后续版本支持）
...
- **0.11 限制**：不支持自定义内存序（如 acquire、release、relaxed）
  - 后续版本可能支持显式内存序参数
...
> **优雅 0.11 原子操作 = `atomic T` 关键字 + 自动原子指令**；  
...
> **优雅 0.11 内存安全 = 所有 UB 必须被编译期证明为安全 → 失败即编译错误**；  
...
2. **`iter(arr: *[T; N]) IteratorT`**
   ...
   - 返回值：返回类型取决于数组元素类型 `T`
     ...
     - 0.11 需要为每种元素类型定义对应的迭代器类型（泛型功能在 v0.12 中提供）
...
flag    = '#' | '0' | '-' | ' ' | '+'
width   = NUM | '*'  // '*' 为未来特性，0.11 不支持
precision = '.' NUM | '.*'  // '.*' 为未来特性，0.11 不支持
...
## 24 优雅 v0.12 泛型增量文档（向后 100% 兼容，零新关键字，无 `where`）

> **注意**：本章描述的是 v0.12 版本的未来特性，当前 0.11 版本尚未实现。  
> — 单页纸，直接附在 0.11 规范末尾 —
...
3. 单态化后走原有 0.11 全部流程（UB 证明、drop、atomic、接口检查等）。

------------------------------------------------
2. 语法影子（用户侧 0 新符号）
------------------------------------------------
| 场景 | 0.11 旧写法 | v0.12 泛型写法 | 备注 |
|---|---|---|---|
...
```
单态化时编译器查找 `impl 具体类型 : Add(i32)`，找不到即报错——与 0.11 接口检查**同一套逻辑**。
...
9. 一句话总结
------------------------------------------------
优雅 v0.12 泛型 = **"签名里写个没声明的类型名"** + `impl` 时括号里给实参；零关键字、零符号、零运行时成本，单页纸读完，0.11 代码无缝继续用。

---

## 25 优雅 v0.12 显式宏（可选增量）
——用 `mc` 区分宏与函数，零新关键字——
