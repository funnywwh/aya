...
- **原子类型**（第 12 章）：`atomic T` 关键字，自动原子指令，零运行时锁
- **内存安全强制**（第 13 章）：所有 UB 必须被编译期证明为安全，失败即编译错误
- **并发安全强制**（第 14 章）：零数据竞争，通过路径零指令
- **字符串插值**（第 20 章）：支持 `"a${x}"` 和 `"pi=${pi:.2f}"` 两种形式
...
- 关键字保留：
  ...
  defer errdefer try catch error null interface impl atomic
  ```
...
- 字符串字面量：
  ...
  - **字符串插值**：`"text${expr}text"` 或 `"text${expr:format}text"`，类型为 `[i8; N]`（编译期展开的定长栈数组）
    ...
    - 格式说明符 `spec` 与 C printf 保持一致，详见第 20 章
...
| `&atomic T`  | 原子指针      | 8 B       | 关键字驱动，见第 12 章 |
| `atomic T`      | 原子类型      | sizeof(T) | 语言级原子类型，见第 12 章 |
...
- `*T`：仅用于接口方法签名，表示指针参数，不能用于普通变量声明或 FFI 函数声明
  ...
  - **示例**：接口方法 `fn write(self: *Self, buf: *byte, len: i32)` 中：
    - `*Self` 表示指向实现接口的结构体类型的指针（`Self` 是占位符，编译时替换为具体类型）
...
- 字符串插值（第 20 章）是阿雅语言本身的特性，编译期展开，与 FFI 的格式字符串不同
...
- **整数溢出和除零**（0.9 强制编译期证明）：
  - 整数溢出：必须证明无溢出，证明失败 → **编译错误**
  - 除零错误：常量除零 → **编译错误**；变量 → 必须证明 `y != 0`，证明失败 → **编译错误**
  - 零运行时检查，通过路径零指令
